...
# BE_V1_AUTH_004 : 필수 정보 검증
| 항목 | 내용 |
| --- | --- |
| **요구사항 ID** | BE_V1_AUTH_004 |
| **요구사항명** | 회원가입 후 필수 데이터 누락 검증 |
| **우선순위** | 필수(P1) |
| **설명** | 회원 가입 이후, 해당 계정에 연결된 **키, 몸무게, 생년월일, 성별** 데이터가 있는지 확인하여 접근 권한을 제어한다. |
| **선행조건** | - 로그인을 통해 서비스 전용 액세스 토큰을 발급받음. |
| **후행조건** | - 데이터 존재 여부에 따라 **정보 입력 페이지로 리다이렉트**. |
| **입력** | - 유저 테이블 내 `height`, `weight`, `birthdate`, `gender` 필드 값 |
| **출력** | - **음성 안내**: "정확한 가이드를 위해 추가 정보를 입력해야 합니다. 입력 화면으로 이동합니다." |
| **비즈니스 규칙** | - **필수 정보(키, 몸무게, 생년월일, 성별)가 모두 저장되는 순간**, 메인 대시보드 진입 권한을 활성화함. |
| **예외 처리** | - **입력 페이지에서 데이터 저장 전 브라우저 종료 시**: 재로그인 시 다시 정보 입력 페이지로 강제 리다이렉트.<br>- **소셜 정보와 수동 입력 정보 충돌 시**: 사용자가 **직접 입력한 정보**를 최우선 순위로 둠. |

# BE_V1_AUTH_007: 토큰 갱신
| 항목 | 내용 |
| --- | --- |
| **요구사항 ID** | BE_V1_AUTH_007 |
| **요구사항명** | Access Token 갱신 |
| **우선순위** | 필수 (P1) |
| **설명** | Refresh Token을 사용하여 만료된 Access Token을 갱신한다. |
| **선행조건** | - 유효한 Refresh Token을 보유하고 있어야 한다. |
| **후행조건** | - 새로운 Access Token 발급 |
| **입력** | - **Refresh Token** |
| **출력** | - **새로운 Access Token**<br>- **만료 시간 (expires_in)** |
| **비즈니스 규칙** | - Access Token 유효기간: 15분<br>- Refresh Token 유효기간: 7일<br>- Refresh Token이 유효하면 새 Access Token 발급<br>- Refresh Token 만료 시 재로그인 필요 |
| **예외 처리** | - **Refresh Token 만료 시**: "세션이 만료되었습니다. 다시 로그인해주세요."<br>- **Refresh Token 블랙리스트 등록된 경우**: "이미 로그아웃된 세션입니다. 다시 로그인해주세요." |

# BE_V1_AUTH_008: 기기 인증 (SMS)
| 항목 | 내용 |
| --- | --- |
| **요구사항 ID** | BE_V1_AUTH_008 |
| **요구사항명** | 새 기기 SMS 인증 |
| **우선순위** | 필수 (P1) |
| **설명** | 새로운 기기에서 로그인 시도 시, 등록된 전화번호로 SMS 인증코드를 발송하여 기기를 검증하고 device_hash를 발급한다. |
| **선행조건** | - 로그인 시도 시 새로운 device_id가 감지됨<br>- 사용자 계정에 전화번호가 등록되어 있음 |
| **후행조건** | - SMS 인증 성공 시 device_hash 발급 및 기기 등록<br>- 이후 해당 기기에서 추가 인증 없이 로그인 가능 |
| **입력** | - **phone_number** (SMS 발송용)<br>- **verification_code** (사용자 입력) |
| **출력** | - **인증코드 발송 시**: "인증번호가 발송되었습니다."<br>- **인증 성공 시**: "기기 인증이 완료되었습니다."<br>- **device_hash** |
| **비즈니스 규칙** | - 인증코드: 6자리 숫자<br>- 인증코드 유효시간: 3분<br>- 최대 재시도 횟수: 5회 (5회 초과 시 30분 대기) |
| **예외 처리** | - **잘못된 인증코드**: "인증번호가 올바르지 않습니다. 다시 입력해주세요."<br>- **인증코드 만료**: "인증번호가 만료되었습니다. 재발송을 요청해주세요."<br>- **재시도 횟수 초과**: "인증 시도 횟수를 초과했습니다. 30분 후 다시 시도해주세요." |
...
| **요구사항명** | 운동 데이터 요약 리포트 조회 |
| **우선순위** | 필수 (P1) |
| **설명** | 사용자의 운동 로그(ExerciseLog)를 집계하여 일간/주간/월간/연간 운동 통계를 제공한다.<br>결과는 TTS로 요약 가능한 텍스트 형태를 포함한다. |
| **선행조건** | - 로그인 상태(유효한 Access Token) |
| **후행조건** | - 없음 (조회) |
| **입력** | - period_type: DAILY \| WEEKLY \| MONTHLY \| YEARLY<br>- base_date: YYYY-MM-DD (기준일, 선택) |
| **출력** | - summary_json:<br>  - period_range (start_date, end_date)<br>  - total_exercise_time_sec<br>  - completed_session_count (is_valid=true)<br>  - exercise_count (수행 동작 수)<br>  - category_breakdown (카테고리별 운동 통계)<br>- tts_summary_text:<br>  - 예: "이번 주 운동은 3회, 총 42분입니다." |
| **비즈니스 규칙** | 1. 집계 대상은 기본적으로 is_valid=true인 세션을 포함한다.<br>2. 리포트는 상시 열람 가능하며, 운동 종료 시 로그가 자동 전송된다.<br>3. 결과는 화면 표시용 JSON과 TTS 요약 텍스트를 함께 제공한다. |
| **예외 처리** | - 해당 기간에 유효 로그가 없는 경우:<br>  - 0 값으로 응답하되 "해당 기간에 기록된 운동이 없습니다." 안내 텍스트 제공 |
