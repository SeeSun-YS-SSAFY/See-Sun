    ...
    def _build_normalize_prompt(cls, text: str, field: str) -> str:
        ...
        rules = {
            ...
            "weight": (
                ...
                "- 찾을 수 없거나 불명확하면 normalized는 null로 반환하세요.\n"
                "- 예: \"칠십육 킬로\" -> normalized \"76\"\n"
            ),
            "gender": (
                "필드: 성별(gender)\n"
                "- 입력에서 '성별'만 추출하세요.\n"
                "- 남자/남성/남 이면 normalized는 \"M\"으로 반환하세요.\n"
                "- 여자/여성/여 이면 normalized는 \"F\"으로 반환하세요.\n"
                "- 찾을 수 없거나 불명확하면 normalized는 null로 반환하세요.\n"
            ),
            "birth": (
                "필드: 생년월일(birth)\n"
                "- 입력에서 생년월일을 YYYY-MM-DD 형식으로 반환하세요.\n"
                "- 찾을 수 없거나 불명확하면 normalized는 null로 반환하세요.\n"
            ),
            "phone": (
                "필드: 전화번호(phone)\n"
                "- 입력에서 전화번호를 010-XXXX-XXXX 형식으로 반환하세요.\n"
                "- 찾을 수 없거나 불명확하면 normalized는 null로 반환하세요.\n"
            ),
        }
