    ...
    <div className="h-full flex-col flex">
      ...
      <div className="flex flex-1 flex-col justify-center gap-4 pb-25">
        {categories.map((category) => (
          <Button key={category.category_id} onClick={() => router.push(`/exercise/${category.category_id}?mode=routine`)}>{category.display_name}</Button>
        ))}
        <div className="flex gap-3.5">
          <Button onClick={() => router.push(`/exercise/5?mode=routine`)}>개인 맞춤</Button>
